...
int main() {
  ...
  for (size_t i = 0; i + 1 < kXenonFrequency.size(); ++i) {
    ...
    if (i == 0) [[unlikely]] {
      ...
      assert(R.absorbed_plasma.size() == r.absorbed_plasma.size());
      for (size_t j = 0; j < r.absorbed_plasma.size(); ++j) {
        R.absorbed_plasma[j] += r.absorbed_plasma[j];
      }

      assert(R.absorbed_quartz.size() == r.absorbed_quartz.size());
      for (size_t j = 0; j < r.absorbed_quartz.size(); ++j) {
        R.absorbed_quartz[j] += r.absorbed_quartz[j];
      }

      R.absorbed_mirror += r.absorbed_mirror;
      R.intensity_all += r.intensity_all;
    }
  }
