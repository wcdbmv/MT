#include "xe_xe_sio2_plot_widget.h"
...
XeXeSiO2PlotWidget::XeXeSiO2PlotWidget(QWidget* parent)
    : QChartView{parent},
      chart_{new QChart},
...
void XeXeSiO2PlotWidget::setAxisY(const QString& title) {
  assert(!axis_y_);
...
void XeXeSiO2PlotWidget::setData(const std::vector<Float>& plasma,
  ...
  for (std::size_t i = 1; i < quartz.size(); ++i) {
    *series2_ << QPointF{step * static_cast<Float>(i), quartz[i]};
  }
}
