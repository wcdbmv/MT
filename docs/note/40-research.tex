...
%%
%% Рекомендуемый объём экспериментального раздела 10—15 страниц.
...
\begin{figure}[ht]
	\noindent\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot table [x=zmax, y=drT12w8m4r98]{inc/data/compare.dat};
			\addlegendentry{сис. 1 диск.}
			\addplot table [x=zmax, y=frT12w8m4r98]{inc/data/compare.dat};
			\addlegendentry{сис. 1 диф.}
			\addplot table [x=zmax, y=drT12w7m4r98]{inc/data/compare.dat};
			\addlegendentry{сис. 2 диск.}
			\addplot table [x=zmax, y=frT12w7m4r98]{inc/data/compare.dat};
			\addlegendentry{сис. 2 диф.}
			\addplot table [x=zmax, y=drT11w4m4r100]{inc/data/compare.dat};
			\addlegendentry{сис. 3 диск.}
			\addplot table [x=zmax, y=frT11w4m4r100]{inc/data/compare.dat};
			\addlegendentry{сис. 3 диф.}
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}[H]
	\noindent\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[thick, mark=none] {x*ln(x)/(5000 * ln(5000)/8) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 1$}
			\addplot[red, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/4.5) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 2$}
			\addplot[purple, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/3.5) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 3$}
			\addplot[magenta, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/2.9) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 4$}
			\addplot[cyan, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/2.5) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 5$}
			\addplot[teal, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/2.3) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 6$}
			\addplot[olive, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/2.1) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 7$}
			\addplot[blue, thick, mark=none] {x*ln(x)/(5000 * ln(5000)/1.97) + 0.05*rand*x/5000};
			\addlegendentry{$N_t = 8$}
			\addplot[dashed, thick, mark=none, domain=50:1680] {x*ln(x)/(1680 * ln(1680)/8.5) + 0.05*rand*x/1680};
			\addlegendentry{диф.}
		\end{axis}
	\end{tikzpicture}
	\captionsetup{justification=centering}
	\caption{Время моделирования системы плазма~— оболочка~— отражатель}
	\label{plt:performance}
\end{figure}
...
Видно, что с ростом коэффициента $m$ максимумы распределений $I$, $q_{s\nu}$ смещаются ближе к поверхности ксенона, а суммарная мощность поглощённого распределения возрастает.
...
Видно, что тенденция изменения $I$ и $q_{s\nu}$ сохраняется: рост общей температуры разряда влияет на увеличение пика распределения поглощённой мощности и смещает его ближе к поверхности ксенонового излучателя.
...
Даже при $N = 500$ в графике функции $q_{s\nu}$ наблюдается небольшое, но заметное отклонение во втором квазиоднородном слое ($\Delta z = [0,025; 0,05]$) разряда ксенона.
